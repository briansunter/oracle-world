...
| **Public access** | Enabled — port 443 open | `enable_public_access = false` |
...
The default deployment creates compute + block storage — a lean base you can build on. Enable MySQL and object storage as needed. The storage split (50 GB boot + 150 GB block) uses the full 200 GB free tier. Alternatively, set `boot_volume_size_gb = 200` and `enable_block_volume = false` for simpler single-disk setup.

### Public vs Private Mode

By default, the instance is **public** (`enable_public_access = true`): port 443 (HTTPS) is opened through every layer (VCN security list and instance iptables).

Set `enable_public_access = false` for private mode — zero inbound ports, SSH only.
...
enable_public_access = true           # Default — open ports
enable_public_access = false          # Private — SSH only
...
The compute instance lives in a **public subnet** — OCI NAT gateways aren't included in Always Free, so a private-subnet instance couldn't reach the internet for updates. The instance gets a public IP directly.
